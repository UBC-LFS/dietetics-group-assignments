...
def read_data_and_clean():
    ...
    with open(PATH + 'data2/raw.csv', 'r') as file:
    ...
    max_per_projects = {}
    exceptions = {'C': 1, 'H': 1, 'J': 1, 'W': 1, 'Y': 1}
    for p in projects:
...
def matching_first_round(students, projects, max_per_projects, preferences, rankings, ranking_map):
    ...
    for i in indexes:
        ...
        for j, item in enumerate(proejct_rankings):
            student = item[0]
            pref = item[1]
            if not proposals[student] and pref in [str(k) for k in range(2,20)]:
                if student in []:
                    continue
...
def save(filename, items):
    with open(PATH + '/output2/' + filename, 'w', newline='') as file:
